...
void StateManager::update()
{
	...
	{
		m_transitionAlpha = static_cast<int>(lerp(m_transitionAlpha, 0.f, m_app.timestep));
		if (m_transitionAlpha <= 25)
		{
	...
	{
		m_transitionAlpha = static_cast<int>(lerp(m_transitionAlpha, 255.f, m_app.timestep));
		if (m_transitionAlpha >= 195)
		{
			m_transitionState = TransitionState::NONE;
			m_transitionAlpha = 255;
			m_stack.push(m_registered[m_transitionNextStateId].get());
		}
	}
}
